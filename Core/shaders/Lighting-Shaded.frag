...
vec3 CalculatePBRPoint(PointLightShadowUniforms light, vec3 n, vec3 objectColor);
// forward /////////////////////////////////////////////////////////////////////
...
void main(void)
{
  ...
  PointLightShadowUniforms l = u_pointLightShadows[v_InstanceID];
  ...
  // computes shadow
  vec3 fragToLight = g_position - l.position;
  float closestDepth = texture(samplerCube(l.shadowMapHandle), fragToLight).r;
  closestDepth *= l.farPlane;
  float currentDepth = length(fragToLight);
  float bias = max(0.05 * (1.0 - dot(g_normal, fragToLight)), 0.005);  
